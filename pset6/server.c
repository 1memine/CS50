...
char* indexes(const char* path) {
    ...
    char *i_list[2] = {"index.php", "index.html"};
    char *c = NULL;
    for (int i = 0; i < 2; i++) {

        if (!c) {
            c = malloc( strlen(path) + 1 + strlen( *(i_list+i) ) );
        } else {
            c = realloc( c, strlen(path) + 1 + strlen( *(i_list+i) ) );
        }
        sprintf(c, "%s/%s", path, *(i_list+i) );
        if (!access(c, F_OK) ) {
            return c;
        }
    }
 ...
 */
const char* lookup(const char* path) {
    if (!strcmp (strrchr(path, '.'), ".css") ) {
        return "text/css";
    } else if (!strcmp (strrchr(path, '.'), ".html") ) {
        return "text/html";
    } else if (!strcmp (strrchr(path, '.'), ".gif") ) {
        return "image/gif";
    } else if (!strcmp (strrchr(path, '.'), ".ico") ) {
        return "image/x-icon";
    } else if (!strcmp (strrchr(path, '.'), ".jpg") ) {
        return "image/jpeg";
    } else if (!strcmp (strrchr(path, '.'), ".js") ) {
        return "text/javascript";
    } else if (!strcmp (strrchr(path, '.'), ".php") ) {
        return "text/x-php";
...
bool parse(const char* line, char* abs_path, char* query) {
    ...
    // Check for correct beginning of request-target
    if ( *(strchr(line, ' ')+1) != '/' ) {
        error(501);
        return false;
    }
    ...
    // Copy request target to abs_path
...
bool load(FILE *file, BYTE **content, size_t *length) {
    ...
    *length = 0;
    BYTE *c = malloc(sizeof(BYTE));
    *content = malloc(sizeof(BYTE));
    while(fread(c, sizeof(BYTE), 1, file) == 1) {
        *length += 1;

        *content = realloc(*content, *length);

        memcpy(*content+*length-1, c, 1);
    }
